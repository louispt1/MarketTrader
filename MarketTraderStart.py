...
class Market:
    ...
    def __init__(self, marketName: str):
        ...
        self.traders = set() # We can use a set instead of a list for faster look-up
        self.active_bids: typing.Dict[int: typing.List[Order]] = dict() # This maps a bid (request to buy) order's price (integer, in cents) to a list of orders at that price
        self.active_asks: typing.Dict[int: typing.List[Order]] = dict() # This maps an ask (request to sell) order's price (integer, in cents) to a list of orders at that price
        self.best_bid_price = 0 # The best bid price is the price of the highest bid order
        self.best_ask_price = float('inf') # The best ask price is the price of the lowest ask order
